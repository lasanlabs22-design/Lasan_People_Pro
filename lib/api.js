...
export async function load(path, options) {
  try {
    ...
    if (err instanceof ApiError) {
      ...
      if (err.status === 403 && /suspended/i.test(err.message)) redirect("/logout?reason=suspended");
    }
    throw err;
  }
}
...
export async function loadPlatform(path, options) {
  try {
    ...
    if (err instanceof ApiError) {
      if (err.code === "password_change_required") redirect("/platform/password");
      if (err.status === 401 || err.status === 403) redirect("/platform/logout");
    }
    throw err;
  }
}
