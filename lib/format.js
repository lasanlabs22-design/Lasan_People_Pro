...
// en-IN abbreviates September as "Sept"; every other month is three letters, so match them.
export const threeLetterMonths = (s) => s.replace(/\bSept\b/, "Sep");

export function fmtDate(iso, opts = { day: "numeric", month: "short", year: "numeric" }) {
  if (!iso) return "—";
  return threeLetterMonths(parse(iso).toLocaleDateString("en-IN", opts));
}

/** A timestamp as its calendar day in the app's time zone, e.g. "26 Sep 2026"; null when missing. */
export const fmtDay = (ts) => (ts ? fmtDate(new Date(ts).toLocaleDateString("en-CA", { timeZone: TZ })) : null);
